...
func (sm2 *SM2Algorithm) CalculateNextReview(card *Card, rating int) int {
	...
	if rating < 3 {
		...
		if card.LastReviewed.IsZero() || card.ReviewInterval == 0 {
			if rating == 3 {
				interval = 1
			} else if rating == 4 {
				interval = 3
...
func (sm2 *SM2Algorithm) CalculateDueDate(card *Card) time.Time {
...
func (sm2 *SM2Algorithm) CalculatePercentOverdue(card *Card) float64 {
	...
	dueDate := sm2.CalculateDueDate(card)
	if time.Now().Before(dueDate) {
		return 0.0
	}

	// Calculate overdue percentage
	intervalDuration := time.Duration(card.ReviewInterval) * 24 * time.Hour
...
func (sm2 *SM2Algorithm) EstimateEase(card *Card) float64 {
	// Simple implementation: scale from difficulty (0-5)
	// 0 = most difficult (ease = 1.3), 5 = easiest (ease = 3.0)
	baseEase := 1.3
	difficultyFactor := float64(card.Difficulty) / 5.0
